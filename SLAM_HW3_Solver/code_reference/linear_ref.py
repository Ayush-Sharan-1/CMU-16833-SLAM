...
def create_linear_system(
    ...
    # print(n_odom, n_obs, n_poses, n_landmarks, M, N)
    # print(sqrt_inv_odom)
    # print(sqrt_inv_obs)


    o00, o01 = sqrt_inv_odom[0, 0], sqrt_inv_odom[0, 1]
    o10, o11 = sqrt_inv_odom[1, 0], sqrt_inv_odom[1, 1]
    
    l00, l01 = sqrt_inv_obs[0, 0], sqrt_inv_obs[0, 1]
    l10, l11 = sqrt_inv_obs[1, 0], sqrt_inv_obs[1, 1]
    ...
    for i in range(n_odom):
        # 1st row of measurement
        A[2+2*i, 2*i]   = -o00
        A[2+2*i, 1+2*i] = -o01
        A[2+2*i, 2+2*i] =  o00
        A[2+2*i, 3+2*i] =  o01
        
        # 2nd row of measurement
        A[3+2*i, 2*i]   = -o10
        A[3+2*i, 1+2*i] = -o11
        A[3+2*i, 2+2*i] =  o10
        A[3+2*i, 3+2*i] =  o11
        ...
        # b vector
        b[2+2*i : 4+2*i] = sqrt_inv_odom @ odoms[i]

    # Then fill in landmark measurements
    obs_offset = 2 + 2 * n_odom      # Priors + odoms
    ...
    for j in range(n_obs):
        ...
        # 1st row of the measurement
        A[r, c_pose]   = -l00
        A[r, c_pose+1] = -l01
        A[r, c_lm]     =  l00
        A[r, c_lm+1]   =  l01
        
        # 2nd row of the measurement
        A[r+1, c_pose]   = -l10
        A[r+1, c_pose+1] = -l11
        A[r+1, c_lm]     =  l10
        A[r+1, c_lm+1]   =  l11
        ...
        b[r:r+2] = sqrt_inv_obs @ observations[j, 2:4]
